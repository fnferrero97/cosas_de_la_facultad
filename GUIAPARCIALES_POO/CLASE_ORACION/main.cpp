...
public:
    ...
    int cuentaLetras(){
        ...
        for (size_t i = 0; i < this->palabras.length(); i++){
            if(!isspace(this->palabras[i])){
               contador++;
            }
        }
        return contador;
    ...
    int cuentaPalabras(){
        ...
        for(size_t i = 1; i < this->palabras.length(); i++){
            if(this->palabras[i-1] == ' ' && isalnum(this->palabras[i])){
                contador++;
            }
        }
    ...
    friend ostream& operator<<(ostream &COUT, Oracion &oracion){
        COUT << oracion.palabras << endl;
    }

};

int main(int argc, char *argv[]){
    Oracion* oracion = new Oracion("Fua sandra, esto es una frase!");

    cout << oracion->cuentaPalabras() << endl;
    cout << oracion->cuentaLetras() << endl;
    cout << *oracion << endl;
